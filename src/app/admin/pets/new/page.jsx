...
export default function NewPetPage() {
  ...
  const [form, setForm] = useState({
    ...
    petLocation: "",      // optional: matches edit page shape
    shelter: "",          // optional: free text
    distanceKm: "",       // optional
  });
  ...
  async function onSubmit(e) {
    ...
    setSaving(true);
    try {
      // Map your UI fields to the API the server expects
      const payload = {
        name: form.name,
        species: form.species,
        breed: form.breed,
        age: form.ageMonths, // your schema uses months (number)
        gender: form.gender,
        size: form.size,
        images: photos, // <-- convert photos[] to images[]
        ...
        distanceKm:
          form.distanceKm === "" ? undefined : Number(form.distanceKm),
      };
          ...
          <div className="p-6 space-y-6">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="Age (months)" hint="Use numeric months (e.g., 6, 24)">
                <input
                  ...
                  onChange={(e) =>
                    set("ageMonths", Number(e.target.value || 0))
                  }
                />
              </Field>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="Tags (comma separated)" hint="e.g., vaccinated, friendly, house-trained">
                <input
                  ...
                  placeholder="gentle, playful, calm"
                />
              </Field>
            </div>
            ...
            {tagsArr.length ? (
              <div className="flex flex-wrap gap-2">
                {tagsArr.map((t, i) => (
                  <span key={`${t}-${i}`} className="badge badge-ghost">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <aside className="rounded-2xl bg-white shadow ring-1 ring-black/5">
          ...
          <div className="p-6 space-y-4">
            ...
            {tagsArr.length ? (
              <div className="flex flex-wrap gap-2">
                {tagsArr.map((t, i) => (
                  <span key={`pv-${t}-${i}`} className="badge badge-outline">
                    {t}
                  </span>
                ))}
              </div>
