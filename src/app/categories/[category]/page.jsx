...
export default function CategoryPage() {
  ...
  // Get all species under the category
  ...
  const filteredPets =
    selectedBreed && petsData.filter((pet) => pet.breed === selectedBreed);

  return (
    <div className="min-h-screen p-4 md:p-6 max-w-6xl mx-auto" style={{ backgroundColor: "rgb(249, 242, 229)" }}>
      {/* Header with Pawfect Match branding */}
      <header className="mb-8 pt-4">
        ...
        <div className="bg-white rounded-xl p-6 shadow-md">
          <h2 className="text-3xl font-bold capitalize text-center text-gray-800">{category}</h2>
      ...
      {!selectedBreed && (
        <div>
          <h3 className="text-xl font-semibold mb-6 text-center text-gray-700">Select a Breed</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {allBreeds.map((breed) => (
              ...
              >
                <div className="w-16 h-16 rounded-full mb-3 flex items-center justify-center" style={{ backgroundColor: "rgb(255, 219, 90)" }}>
                  <span className="text-white text-2xl">🐾</span>
                </div>
                <h3 className="text-lg font-semibold text-center text-gray-800">{breed}</h3>
                <p className="text-sm text-gray-500 mt-1 text-center">View available pets</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 p-4 rounded-xl bg-white shadow-md">
            <h3 className="text-xl font-semibold text-gray-800 mb-4 sm:mb-0">
              Available <span className="text-amber-500">{selectedBreed}</span> {category.toLowerCase()}s
            </h3>
            <button
          ...
          {filteredPets && filteredPets.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {filteredPets.map((pet) => (
                ...
                >
                  ...
                  <h4 className="font-bold text-xl mb-2 text-gray-800">{pet.petName}</h4>
            ...
            <div className="text-center py-12 rounded-xl bg-white shadow-md">
              <div className="w-24 h-24 mx-auto mb-4 rounded-full flex items-center justify-center" style={{ backgroundColor: "rgb(255, 219, 90)" }}>
                <span className="text-4xl">🐕</span>
              </div>
              <h3 className="text-xl font-semibold text-gray-800 mb-2">No pets found</h3>
              <p className="text-gray-600 mb-6">We don't have any {selectedBreed} {category.toLowerCase()}s available right now.</p>
              <button
                onClick={() => setSelectedBreed(null)}
      ...
      <footer className="mt-12 pt-6 border-t border-gray-200 text-center text-gray-600">
        <p>© {new Date().getFullYear()} Pawfect Match. Find your perfect pet companion.</p>
      </footer>
    </div>
  );
}