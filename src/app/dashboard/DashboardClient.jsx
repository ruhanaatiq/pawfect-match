...
function Sparkline({ points = [], className = "text-emerald-500" }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className={`w-full h-24 ${className}`} role="img">
      <path d={d} stroke="currentColor" strokeWidth="2.5" fill="none" strokeLinecap="round" />
      {points.map((v, i) => <circle key={i} cx={i * dx} cy={ny(v)} r="2.5" fill="currentColor" />)}
    </svg>
  );
}
...
export default function DashboardClient({ initialTab = "profile" }) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      } catch (e) { console.error(e); toast.error("Couldn't load favorites."); }
    }
    run();
    return () => { cancelled = true; };
  }, [activeTab, userEmail]);
  ...
  useEffect(() => {
    ...
    async function fetchWorkshops() {
      ...
      } catch (e) { console.error(e); toast.error("Failed to load workshops"); }
    }
    fetchWorkshops();
    return () => { cancelled = true; };
  }, []);
  ...
  applications.forEach(app => {
    ...
    if (t && !Number.isNaN(t.getTime())) map.set(t.toISOString().slice(0, 10), (map.get(t.toISOString().slice(0, 10)) || 0) + 1);
  });
  const weekly = keys.map(k => map.get(k) ?? 0);
  const trendColor = weekly[weekly.length - 1] >= weekly[0] ? "text-emerald-500" : "text-red-500";

  const workshopList = useMemo(() => workshops.slice().sort((a, b) => new Date(a.date) - new Date(b.date))
    .map(ws => ({ ...ws, countdown: getCountdown(ws.date, now), joined: !!joinedWorkshops[ws.id] })), [workshops, joinedWorkshops, now]);

  /* ---------- Handlers ---------- */
  const handleProfileUpdate = async () => {
    try {
      const res = await fetch("/api/update-profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(profileData) });
      if (!res.ok) throw new Error("Failed to update profile");
      await update({ user: { ...session.user, ...profileData } });
      toast.success("Profile updated!"); setUpdateModalOpen(false);
  ...
  const confirmCancel = async () => {
    ...
    } catch (e) { console.error(e); toast.error("Failed to cancel application."); }
    finally { setModalOpen(false); setSelectedApp(null); }
  };
  ...
  const removeFavorite = async (favId) => {
    ...
    } catch (e) { console.error(e); toast.error("Failed to remove favorite"); }
  };
  ...
  const downloadApplications = () => {
    ...
    const rows = applications.map(app => [
      app.petName, app.status, app.applicant?.city || "N/A", app.applicant?.phone || "N/A",
      app.createdAt ? new Date(app.createdAt).toLocaleDateString() : ""
    ]);
    const csvContent = [headers, ...rows].map(e => e.join(",")).join("\n");
    const blob = new Blob([csvContent], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "my_applications.csv"; a.click();
  ...
  if (!session) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-emerald-50 to-white">
      ...
      <Link href="/login" className="px-5 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 shadow">Go to Login</Link>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen bg-[url('/paws-bg.png')] bg-repeat bg-emerald-50/50">
      {/* Sidebar + Hamburger */}
      <button onClick={() => setSidebarOpen(v => !v)} className="md:hidden fixed top-4 left-4 z-50 p-2 rounded-md bg-white shadow" aria-label="Toggle menu"><FaBars className="text-emerald-600" /></button>
      <aside className={`fixed inset-y-0 left-0 w-60 p-6 transform transition-transform duration-300 z-40 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 md:flex md:flex-col bg-white/30 backdrop-blur-lg border-r border-white/20 shadow-lg`}>
        <div className="flex justify-end md:hidden mb-4"><button onClick={() => setSidebarOpen(false)} className="text-gray-500 hover:text-gray-700 text-2xl font-bold">&times;</button></div>
        {[{ tab: "profile", icon: <FaUser className="mr-2" />, label: "Profile" }, { tab: "applications", icon: <FaPaw className="mr-2" />, label: "Applications" }, { tab: "favorites", icon: <FaHeart className="mr-2" />, label: "Favorites" }, { tab: "my-bookings", icon: <FaCalendarCheck className="mr-2" />, label: "My Bookings" }, { tab: "my-feedback", icon: <FaComments className="mr-2" />, label: "My Feedback" }, { tab: "users-reviews", icon: <FaStar className="mr-2" />, label: "Reviews" }, { tab: "sponsorships", icon: <FaHandshake className="mr-2" />, label: "Sponsorships" }, { tab: "settings", icon: <FaCog className="mr-2" />, label: "Settings" }].map(btn => (
          <button key={btn.tab} onClick={() => { setActiveTab(btn.tab); setSidebarOpen(false); }} className={`flex items-center px-3 py-2 rounded-2xl mb-3 ${activeTab === btn.tab ? "bg-emerald-600 text-white" : "text-gray-700 hover:bg-emerald-100"}`}>{btn.icon} {btn.label}</button>
        ))}
        <button onClick={signOut} className="flex items-center px-3 py-0 rounded-lg mt-4 text-red-600 hover:bg-red-100"><FaSignOutAlt className="mr-2" /> Logout</button>
        <div className="mt-auto flex justify-center relative"><img src="welcomedog.png" alt="Cute Dog Hug" className="w-60 h-85 rounded-2xl relative z-10" /><div className="absolute inset-0 rounded-2xl pointer-events-none" /></div>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-8 overflow-y-auto transition-all duration-300">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 space-y-6">
              <div className="mb-6 p-6 rounded-3xl bg-gradient-to-br from-amber-100 via-pink-100 to-rose-100 shadow-lg ring-1 ring-rose-200 text-center relative overflow-hidden">
                ...
                <div className="relative inline-block">
                  <img src={user.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || "User")}&background=FBCFE8&color=4C3D3D`} alt="Profile" className="w-24 h-24 rounded-full border-4 border-pink-200 shadow-md mx-auto mb-3" />
                  <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-1 shadow">🐕</div>
                </div>
                <h2 className="text-2xl font-bold text-[#4C3D3D] mt-2">Welcome back, {user.name || "User"}!</h2>
              ...
              <div className="rounded-2xl bg-white/90 shadow-sm ring-1 ring-black/5 p-4">
                ...
                <div className="mt-2"><Sparkline points={weekly} className={trendColor} /></div>
              </div>
            </div>
            ...
            <div className="lg:w-72 flex flex-col gap-4">
              ...
              <div className="rounded-2xl bg-amber-100 shadow-sm ring-1 ring-black/5 p-4 border border-emerald-100 h-50 overflow-y-auto">
                ...
                {workshopList.map(ws => (
                  <div key={ws.id} className="relative bg-cover bg-center rounded-xl shadow overflow-hidden h-28 mb-3" style={{ backgroundImage: `url(${ws.bgImage})` }}>
                    ...
                    <div className="relative z-10 p-3 flex flex-col justify-between h-full text-white">
                      ...
                      <div className="flex justify-between items-center mt-1">
                        ...
                        <button onClick={() => router.push(`/workshops/${ws.id}`)} className="px-2 py-1 text-xs rounded-lg bg-emerald-600 hover:bg-emerald-700">Join</button>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {activeTab === "applications" && (
          <section>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={downloadApplications} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700"><FaDownload /> Download CSV</button>
            </div>
            ...
            {applications.length === 0 ? (
              ...
              <ul className="space-y-4">
                {applications.map((app) => {
                  ...
                  return (
                    <li key={app._id} className="p-5 bg-white rounded-3xl shadow-lg flex flex-col md:flex-row md:justify-between md:items-center w-full hover:scale-102 hover:shadow-2xl transition-transform duration-300">
                      <div className="flex-1 space-y-2 mt-4 md:mt-0">
                        <p className="font-semibold text-lg text-[#4C3D3D]">{app.petName || "Unknown Pet"}</p>
                        <p className="text-sm text-gray-600">Status: <span className={`font-medium px-2 py-0.5 rounded-full text-xs ${status === "approved" ? "bg-green-100 text-green-700" : status === "pending" ? "bg-yellow-100 text-yellow-700" : "bg-gray-200 text-gray-600"}`}>{app.status}</span></p>
                        ...
                        <div className="mt-2 w-full bg-gray-200 h-3 rounded-full overflow-hidden"><div className={`h-3 rounded-full ${bar}`} style={{ width }} /></div>
                      </div>
                      {status === "pending" && <button onClick={() => { setSelectedApp(app); setModalOpen(true); }} className="mt-4 md:mt-0 md:ml-6 flex-shrink-0 px-4 py-2 bg-red-600 text-white rounded-xl shadow hover:bg-red-700 transition">Cancel</button>}
                    </li>
                  );
                })}
              </ul>
        ...
        {activeTab === "users-reviews" && <FeedbackCards limit={null} showHeader={false} grid={2} />}
        {activeTab === "sponsorships" && <SponsorshipRequests userEmail={userEmail} />}
        {activeTab === "settings" && (
          <section>
            ...
            <div className="space-y-3 p-6 rounded-2xl bg-gradient-to-br from-emerald-50/70 via-white/60 to-rose-50/50 backdrop-blur shadow-xl">
              <button onClick={() => setUpdateModalOpen(true)} className="block text-emerald-600 hover:underline">Update Profile</button>
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white rounded-xl p-6 w-80 relative z-10">
            ...
            <p className="mb-6 text-gray-600">Are you sure you want to cancel your adoption for <span className="font-medium">{selectedApp?.petName || "this pet"}</span>?</p>
      ...
      {updateModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white rounded-xl p-6 w-80 relative z-10">
            ...
            <input type="text" value={profileData.name} onChange={e => setProfileData({ ...profileData, name: e.target.value })} className="w-full mb-4 px-3 py-2 border rounded-lg" />
            <label className="block mb-2 text-sm font-medium text-gray-700">Email</label>
            <input type="email" value={profileData.email} onChange={e => setProfileData({ ...profileData, email: e.target.value })} className="w-full mb-4 px-3 py-2 border rounded-lg" />
            <div className="flex justify-end space-x-3">
              <button onClick={() => setUpdateModalOpen(false)} className="px-4 py-2 rounded-lg bg-gray-300 hover:bg-gray-400">Cancel</button>
              <button onClick={handleProfileUpdate} className="px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">Save</button>
            </div>
          </div>
        </div>
      )}
