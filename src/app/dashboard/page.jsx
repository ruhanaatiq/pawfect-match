...
import { FaHeart, FaPaw, FaCog, FaSignOutAlt, FaUser, FaBars } from "react-icons/fa";
...
export default function DashboardPage() {
  ...
  async function confirmCancel() {
    ...
    try {
      ...
      setApplications(applications.filter(app => app._id !== selectedApp));
  ...
  return (
    <div className="flex min-h-screen bg-[url('/paws-bg.png')] bg-repeat bg-emerald-50/50">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 w-60 bg-white shadow-lg p-6 transform md:relative md:translate-x-0 transition-transform duration-300 z-40
        ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:flex flex-col`}
      >
