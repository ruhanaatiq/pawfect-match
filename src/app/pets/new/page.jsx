...
export default function NewPetPage() {
  ...
  const shelterId = sp.get("shelter") || ""; // passed from Shelter/Admin buttons
  ...
  const [form, setForm] = useState({
    ...
    petLocation: "",
    shelter: "",          // free text label (optional)
    distanceKm: "",       // optional
  });

  const set = (k, v) => setForm((s) => ({ ...s, [k]: v }));

  const photos = useMemo(() => (Array.isArray(form.photos) ? form.photos : []), [form.photos]);
  ...
  async function onSubmit(e) {
    ...
    try {
      const payload = {
        ...
        breed: form.breed,
        age: form.ageMonths,                // months (number)
        gender: form.gender,
        size: form.size,
        ...
        shelterId: shelterId || undefined,  // IMPORTANT for API
      ...
      <form onSubmit={onSubmit} className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl bg-white shadow ring-1 ring-black/5">
          ...
          <div className="px-4 py-3 border-t bg-base-50 flex sm:hidden justify-end gap-2 rounded-b-2xl">
            ...
            <button className={`btn btn-success ${saving ? "btn-disabled" : ""}`} disabled={saving}>{saving ? "Saving…" : "Create"}</button>
          </div>
        </div>
