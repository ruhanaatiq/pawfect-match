...
const AvailablePetsCard = ({pet}) => {
  // console.log(pet)
  const {image, petName,name
, age, temperament, longDescription,description, breed, id} = pet
  return (
    <div className="bg-white rounded-2xl shadow-md hover:shadow-emerald-100 duration-500 overflow-hidden hover:shadow-lg transition flex flex-col h-[440px]">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <p className="text-gray-600 text-sm mt-2 flex-1 ">
          {temperament || (description && description.length > 90 
            ? description.slice(0, 90) + '...' 
            : description) || 'A lovely pet looking for a home!'}
        </p>

        <div className=" flex justify-end items-end">
          <Link href={`/pets/${id}`} className="mt-4 inline-flex items-center bg-emerald-600 text-white px-4 py-1 sm:py-2 rounded-full text-sm font-medium hover:bg-emerald-500 transition cursor-pointer">
